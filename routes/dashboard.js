...
router.get('/', requireAuth, async (req, res) => {
  try {
    ...
    const todaySales = await Invoice.aggregate([{ $match: { date: { $gte: today, $lt: tomorrow } } },{ $group: { _id: null, total: { $sum: '$total' }, count: { $sum: 1 }, paid: { $sum: '$paid' } } }]);
    const monthSales = await Invoice.aggregate([{ $match: { date: { $gte: monthStart } } },{ $group: { _id: null, total: { $sum: '$total' }, count: { $sum: 1 } } }]);
    ...
    const outstanding    = await Invoice.aggregate([{ $match: { balance: { $gt: 0 } } },{ $group: { _id: null, total: { $sum: '$balance' }, count: { $sum: 1 } } }]);
    const outstandingPO  = await Purchase.aggregate([{ $match: { balance: { $gt: 0 } } },{ $group: { _id: null, total: { $sum: '$balance' }, count: { $sum: 1 } } }]);

    const recentInvoices = await Invoice.find().sort({ date: -1 }).limit(5).populate('customer','name');

    const sevenDaysAgo = new Date(today); sevenDaysAgo.setDate(sevenDaysAgo.getDate()-6);
    const salesTrend = await Invoice.aggregate([{ $match: { date: { $gte: sevenDaysAgo } } },{ $group: { _id: { $dateToString: { format: '%Y-%m-%d', date: '$date' } }, total: { $sum: '$total' }, count: { $sum: 1 } } },{ $sort: { _id: 1 } }]);
    ...
    const vatInv = await Invoice.aggregate([{ $match: { date: { $gte: monthStart }, status: { $ne: 'draft' } } },{ $group: { _id: null, outputVAT: { $sum: '$vatAmount' } } }]);
